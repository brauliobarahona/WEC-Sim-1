...
evalc('wecSimInputFile_batch_INIT');
toc;
...
sea = SeaState(seastatename);  % select sea state

[Hs, Tp, JDP] =sea.set4loop;   % discard Hs, Tp values with zero occurence
...
%                     CPTOf3_Cd497
%                     CPTOf1_Cd27599
Cpto = interp2( sea_pto.Tp, sea_pto.Hs, CPTOf1_Cd27599, Tp, Hs );
...
% (!) tweak the start of simulations
JJ0 = 1;

% (3) Loop simulations
for jj = JJ0:length(Cpto)
    
    % update simulation parameters for each case
    waves.H = Hs(jj);     % wave significant height [m]
    waves.T = Tp(jj);     % wave peak perido [s]
    
    % TODO: save the actual values of Hs and Tp for each batch, to save
    % headaches when tweak the start of the batch. For now this is fixed
    % with the script check4files, which will check if output files exist
    % and save the corresponding parameters when they do exist.
    %
...
%
% NOTE: Hs, Tp are the values set for the batch, if the start is tweaked
